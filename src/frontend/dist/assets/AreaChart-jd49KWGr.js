import { c as createLucideIcon, aa as useBackend, ab as useQuery, ac as commonjsGlobal, a9 as getDefaultExportFromCjs, r as reactExports, a7 as clsx, a8 as React, ad as invariant } from "./index-D0FyCSZw.js";
/**
...
function todayRange() {
  ...
  const startOfDay = BigInt((/* @__PURE__ */ new Date()).setHours(0, 0, 0, 0)) * BigInt(1e6);
  return { fromTime: startOfDay, toTime: now2 };
}
function last30DaysRange() {
  const now2 = BigInt(Date.now()) * BigInt(1e6);
  const past = BigInt(Date.now() - 30 * 24 * 60 * 60 * 1e3) * BigInt(1e6);
  return { fromTime: past, toTime: now2 };
}
function useSalesSummary(range3) {
  const { actor, ready } = useBackend();
  const r = range3 ?? todayRange();
  return useQuery({
...
function usePurchaseSummary(range3) {
  const { actor, ready } = useBackend();
  const r = range3 ?? last30DaysRange();
  return useQuery({
...
function useGstBreakdown(range3) {
  const { actor, ready } = useBackend();
  const r = range3 ?? last30DaysRange();
  return useQuery({
...
function _toConsumableArray$8(arr) {
  return _arrayWithoutHoles$8(arr) || _iterableToArray$9(arr) || _unsupportedIterableToArray$e(arr) || _nonIterableSpread$8();
}
...
function _slicedToArray$6(arr, i) {
  return _arrayWithHoles$7(arr) || _iterableToArrayLimit$6(arr, i) || _unsupportedIterableToArray$b(arr, i) || _nonIterableRest$7();
}
...
export {
  ...
  Dot as D,
  getMaxRadius as E,
  getPercentValue as F,
  Global as G,
  warn as H,
  generateCategoricalChart as I,
  Bar as J,
  formatAxisMap2 as K,
  Layer as L,
  formatAxisMap$1 as M,
  usePurchaseSummary as N,
  Legend as O,
  useGstBreakdown as P,
  ...
  Text as h,
  isFunction$3 as i,
  getTickClassName as j,
  adaptEventsOfChild as k,
  Label as l,
  Curve as m,
  isNil$1 as n,
  getValueByDataKey as o,
  polarToCartesian as p,
  Animate as q,
  get$2 as r,
  interpolateNumber$2 as s,
  isEqual$1 as t,
  useSalesSummary as u,
  isNumber as v,
  LabelList as w,
  uniqueId as x,
  mathSign as y,
